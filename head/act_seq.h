...
#include "build/_deps/cute-src/include/cute_coroutine.h"
...
/// - 当调用 `play` 时，为指定的 `BaseObject*` 创建并启动一个 Coroutine，按动作链顺序执行动作；每个动作在开始时被调用一次，然后在后续若干帧内保持“等待”（通过协程 yield）。
...
/// - 动作回调签名为 `std::function<void(BaseObject*)>`；回调在主线程（调用 `main_thread_on_update` 的上下文）执行。
/// - 协程资源：本类在 `play` 内部分配一个上下文指针（在协程结束时由 update-lambda 清理），并在协程死亡时调用 `Cute::destroy_coroutine` 来释放协程句柄。
/// </summary>
class ActSeq {
public:
    using Handler = std::function<void(BaseObject*)>;
    using Frames = int;
    ...
    bool play(BaseObject* obj) {
        ...
        auto coroutine_fn = [](CF_Coroutine co) {
            ...
            for (size_t i = 0; ; ++i) {
                ...
                if (step.h) step.h(c->obj);

                // 等待指定帧数，每帧通过 main_thread_on_update resume
        ...
        CF_Coroutine co = Cute::make_coroutine(coroutine_fn, 0, ctx);
        ctx->co = co;
